...
-**kasir** - bisa membuat transaksi penjualan, melihat prouk
-**owner** - akses penuh,termasuk kelola produk, approve role user baru, dan lihat laporan

User baru harus di-approve oleh owner (`PUTH /api/v1/users/:id/role`) sebelum bisa menggunakan sistem-mencegah sembanrang orang mendapat akses langsung.

## Fitru Utama
- **Autentikasi ganda**: login manual (email/password) dan Google OAuth 2.0 transaction (`BEGIN/COMMIT/ROLLBACK`) dan row locking (`FOR UPDATE`) saat mengurai stok, sehingga dua transaksi bersamaan tidak akan membuat stok minus 
- **Rate limiting**: 5 percobaan/15 menit khusus endpoint login-register (cegah brute force), 100 request/15 untuk endpoint lain
...
1. Clone Repo ini
2. Masuk ke folder `umkm_backend`, dijalankan `npm install`
3. buat database PostgreSQL baru, jalankan skrip SQL di `database/schema.sql`
4. Copy `.env.example` menjadi `.env`, sesuaikan dengan konfigurasi database dan google clinet id kamu sendiri
5. (Opsional) jalankan `node database/seed/seedProduk.js` untuk isi data produk contoh dari dataset Kaggle
